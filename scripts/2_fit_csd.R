...
locations <- locations[locations %in% c("India", "Andhra_Padesh")]

rhat <- matrix(2, length(locations), 2)
rownames(rhat) <- locations
colnames(rhat) <- c("D1", "D2")
...
while (max(rhat) > threshold) {
  for (loc in locations) {
    ...
    while (rhat0 > threshold) {
      ...
      if (rhat1 < rhat0) {
        rhat[loc, 1] <- rhat1
        save(post, file = here::here("out", "sub_cs", "post_csd_d0_" + glue::as_glue(loc) + ".rdata"))
      }
    ...
    while (rhat0 > threshold) {
      ...
      if (rhat1 < rhat0) {
        rhat[loc, 2] <- rhat1
        save(post, file = here::here("out", "sub_csd", "post_csd_d1_" + glue::as_glue(loc) + ".rdata"))
      }
      cat(min(rhat1, rhat0)); cat("\n")
    }
  }

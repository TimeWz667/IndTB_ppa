...
for (scenario in c("shared_pr_asym", "shared_r_onset")) {
  for (cnr_year in 2019:2021) {
    ...
    file_cascades <- glue::as_glue("cascades_") + scenario + ".rdata"
    ...
    drop_n <- cascades %>% 
      mutate(
        across(starts_with("Drop"), function(x) x * Burden)
      ) %>% 
      select(Location, starts_with("Drop")) %>%
      pivot_longer(-Location, names_to = "Stage") %>% 
      group_by(Location, Stage) %>% 
    ...
    g <- tree_dead %>% 
      ...
      labs(title = "Deaths due to TB by stage",
           caption = sprintf("Year: %d, Assumption: %s", cnr_year, scenario))
    
    ggsave(g, filename = here::here("results", folder, "g_tree_dead" + ext), width = 8, height = 8)  
    ...
    ggsave(g, filename = here::here("results", folder, "g_dropout" + ext), width = 8, height = 8)  
  }
}

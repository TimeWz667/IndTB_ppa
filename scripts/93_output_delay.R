...
for (scenario in c("shared_pr_asym", "shared_r_onset")) {
  for (cnr_year in 2019:2021) {
    ...
    delay %>% 
      ...
      arrange(Location) %>% 
      write_csv(here::here("results", folder, "Delay.csv"))
    ...
    ggsave(g, filename = here::here("results", folder, "g_tte") + ext, width = 7, height = 8)  
    ...
    ggsave(g, filename = here::here("results", folder, "g_spent" + ext), width = 7, height = 8)  
